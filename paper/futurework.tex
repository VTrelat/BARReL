...
Finally, the artifacts also contain all examples from the paper and more that exercise \barrel's workflow.
...
Although \barrel is a proof-producing backend for the obligations it emits, it currently still relies on Atelier~B's \texttt{bxml} and \texttt{pog} binaries for parsing B machines and generating proof obligations.
A natural next step is therefore to implement, \emph{in Lean}, a PO generator for the supported fragment that produces Lean propositions directly over \barrel's embedding---with correct handling of WD conditions---and to establish soundness guarantees stating that discharging these obligations entails invariant preservation and refinement correctness.
...
Second, this paves the way to developing an embedded DSL for writing B developments directly in Lean.
