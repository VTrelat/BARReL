...
\fixme[This is redundant: it has been said one paragraph before (the item list)]
The overall engineering effort for extending \barrel may be summarized as follows: extend the syntax with the new symbol, extend the parser to recognize it and write the corresponding encoding rule. If the operator is partial, a corresponding WD condition must also be defined and generated where appropriate.
...
Finally, the artifacts also contain examples from the paper and more that exercise the end-to-end \fixme[?] workflow.
...
Since \barrel operates entirely inside Lean, such automation can remain \emph{proof-producing}, meaning that such tactics can synthesize proof terms which are still available for user inspection \todo[Is this really important? Nobody reads the generated proof terms...unless for debugging purposes].
