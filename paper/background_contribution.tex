...
These partial operators are typically encoded using Hilbert's bounded choice operator $\epsilon\ x \in S.\ P$ which is unspecified\todo[Why underspecified?] if no element of $S$ satisfy $P$.
	...
	$\metavar{m_1}$ is finally assigned $min\_wd_1$, and substituted in $P$ to obtain the final goal to be proved by the user: \[
		\begin{split}
			\vdash \forall\, & (S : \mathtt{Set}\ \mathbb{Z})\, (h_1 : S \in \mathtt{B.Builtins.FIN_1}\ \mathbb{Z})                                       \\
			                 & \quad (h_2 : \forall\, (x : \mathbb{Z})\, (h_3 : x \in S),\ x \le 0),\ \mathtt{B.Builtins.min}\ S\ (min\_wd_1\ S\ h_1\ h_2) \le 0
		\end{split}
	\]
\end{example}
...
\end{enumerate}
For instance, in the arithmetic fragment used throughout our running examples, the library includes dedicated WD lemmas for the minimum operation, as shown in \cref{lst:min-wd-lemmas}, each of which being tagged with the relevant theorem category.
