...
\paragraph*{But how does it work?}
\barrel is organized into four clearly separate passes.
First, the machine is parsed into the BXML format, and proof obligations are generated from this format using Atelier B's internal tools \texttt{bxml} and \texttt{pog}.
Although \texttt{pog} is able to generate WD conditions for the given machine, we choose to rely on our own generation instead, since the WD conditions that are generated by \texttt{pog} may not exactly match the context in which they will appear in the Lean terms.
Then, the resulting POs (in XML format) are parsed by \barrel into an Abstract Syntax Tree (AST) and normalized.
Normalization ensures that B predicates like $P \wedge Q \Rightarrow R$ are treated the same as $P \Rightarrow Q \Rightarrow R$.
Although it is not needed, we perform normalization of B terms solely for the purpose of generating Lean goals that overall need less conjunction destructions.
The normalized B formulas are then transcribed into Lean terms using Lean's facilities for meta-programming/elaboration.
...
Instead of handling the bookkeeping ourselves, we generate meta-variables (which contain their local contexts that they appear in) in place of the WD conditions.
We later assign the generated meta-variables with fresh theorem names whose propositions are extracted from the context of the meta-variable. \todo[needs further explaining or an example?]
All the previous steps are performed by the command \texttt{import machine <name> from <folder>}.
...
\begin{lstlisting}[language=lean, caption={Some WD lemmas about extrema in our B library.}, label=lst:min-wd-lemmas, float]
  @[grind ., wf_min]
  theorem min.WF_singleton {α : Type _} [PartialOrder α] {a : α} :
    min.WF {a} := ⋯
  @[wf_min]
  theorem min.WF_of_finite {α : Type _} [LinearOrder α] {S A : Set α}
    (h : S ∈ FIN₁ A) : min.WF S := ⋯
  @[grind ., wf_min]
  theorem min.WF_interval {lo hi : ℤ} (h : lo ≤ hi) :
    min.WF (lo..hi) := ⋯
\end{lstlisting}
...
\begin{lstlisting}[language=lean, caption={Some membership lemmas about finite-sets and intervals in our B library.}, label=lst:finset-interval-lemmas, float]
  ...
  theorem min.of_singleton {α : Type _} [PartialOrder α] {a : α} :
      min {a} (min.WF_singleton) = a := ...
  @[grind ., simp]
  theorem interval.min_eq {lo hi : Int} (h : lo ≤ hi) :
      min (lo .. hi) (min.WF_interval h) = lo := ...
\end{lstlisting}
