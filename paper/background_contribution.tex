...
Although \texttt{pog} can generate WD conditions for the given machine, we choose to rely on our own generation instead, since the WD conditions that are generated by \texttt{pog} may not exactly match the context in which they will appear in the Lean terms.
...
\begin{example}
	...
	Let us illustrate how WD conditions are handled by \barrel on the following (already normalized) B predicate: \[
		P := \forall S \cdot (S \in \bfinone{\mathbb{Z}} \Rightarrow (\forall x \cdot x \in S \Rightarrow x \le 0) \Rightarrow \bmin{S} \le 0)
	\]
	%
	...
	\[
		\begin{split}
			P_1 := \forall\, & (S : \mathtt{Set}\, \mathbb{Z})\, (h_1 : S \in \mathtt{B.Builtins.FIN_1}\ \mathbb{Z})                                                 \\
	...
	A fresh theorem $wd$ of type \[
		...
		\end{split}
	\]
	(the type of $\metavar{m_1}$ universally quantified by its local context) is then generated.
	$\metavar{m_1}$ is then simply substituted for this new theorem in the goal to finally obtain the final goal \[
		\begin{split}
			P_2 := \forall\, & (S : \mathtt{Set}\ \mathbb{Z})\, (h_1 : S \in \mathtt{B.Builtins.FIN_1}\ \mathbb{Z})                                       \\
