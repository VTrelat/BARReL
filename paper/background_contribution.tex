...
This section presents \barrel and its workflow as summarised in \cref{fig:barrel-archi}.
We first recall the B method, then explain how \barrel handles well-definedness conditions and finally explain how to use \barrel.
...
In practice, our tool generates WD side goals that are equivalent\footnote{\todo[Isn't it already what is written just before the point of insertion of the footnote?] To be \rw{exactly}{} accurate, our WD conditions are equivalent to theoretical WD conditions from the specification of the B language. The only difference is that our WD conditions are generated from the exact Lean context whereas Atelier~B already performs simplification steps at this point, which may alter the PO.} to the WD side goals generated by Atelier B.
%
Then, we take advantage of the dependent types of Lean to encode partial operators using \texttt{Classical.choose} (Hilbert's $\epsilon$ operator, in Lean) which explicitly depends on a proof that the predicate is satisfied by at least one element\footnote{Lean has no built-in notion of partiality and must therefore exhibit WD conditions as explicit proof parameters of the relevant total functions.}.
...
\begin{example}
	...
	%
	Let us illustrate how WD conditions are handled by \barrel on the following (already normalized) B predicate: \[
	...
	%
	A fresh theorem $wd$ is then generated from the type of $\metavar{m_1}$ universally quantified by its local context: \[
		\begin{split}
			\mathtt{theorem}\ wd : \forall\, & (S : \mathtt{Set}\ \mathbb{Z})\, (h_1 : S \in \mathtt{B.Builtins.FIN_1}\ \mathbb{Z})                 \\
			                                 & \quad (h_2 : \forall\, (x : \mathbb{Z})\, (h_3 : x \in S),\ x \le 0),\ \mathtt{B.Builtins.min.WD}\ S
		\end{split}
	\]
	$\metavar{m_1}$ is finally assigned $wd$, and substituted in $P$ to obtain the final goal to be proved by the user: \[
		\begin{split}
			\vdash \forall\, & (S : \mathtt{Set}\ \mathbb{Z})\, (h_1 : S \in \mathtt{B.Builtins.FIN_1}\ \mathbb{Z})                                       \\
			                 & \quad (h_2 : \forall\, (x : \mathbb{Z})\, (h_3 : x \in S),\ x \le 0),\ \mathtt{B.Builtins.min}\ S\ (wd\ S\ h_1\ h_2) \le 0
		\end{split}
	\]
\end{example}

All the previous steps are performed by the \texttt{import <type> <name> from <folder>} command of our tool.
...
Finally, the command \texttt{prove\_obligations\_of <name>} gives back all the generated WD conditions and propositions to the user for them to prove them.
...
Accordingly, \barrel reduces the trusted computing base for the \emph{translation} step \fixme[uh?].
Concretely, the remaining trust assumptions are:
\begin{itemize}
	\item Lean's kernel, and its standard metatheory, including classical reasoning where used,
