...
\paragraph{Encoding of partial B operators}
Some operators of the B language are said to be partial, meaning that they are only defined when some well-formedness conditions are satisfied.
A typical example of such an operator is the minimum operator $min(S)$, which when given a set $S$ returns its smallest element, if it exists, and is undefined otherwise.
...
We take advantage of the dependent types of Lean to encode partial operators using \texttt{Classical.choose} (Hilbert's $\epsilon$ operator) which explicitly depends on a proof that the predicate is satisfied by at least one element.
This proof, which is the WF condition of the operator, is then generated as a side theorem which must be discharged for the main theorem to be fully proven (i.e.\@ not depend on the \texttt{sorryAx} axiom).