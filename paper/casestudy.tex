...
\footnote{
	The assertion $\exists x, x \in \mathbb{Z} \land x \notin \mathbb{Z}$---which is false in the B method---can be discharged in the interactive prover with the instantiation $x := \min(\varnothing)$ via the tactic \texttt{se} and a call to the automatic internal prover \texttt{pp}.}.
