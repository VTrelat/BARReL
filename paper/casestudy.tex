...
The development computes the minimum of a non-empty finite set of integers and is progressively refined into an index-based traversal of an array,
...
\texttt{MinSearch.mch} (specification),
\texttt{MinSearch\_r1.ref} (first refinement),
\texttt{MinSearch\_r2.ref} (second refinement),
...
The initial machine \texttt{MinSearch} shown in Listing~\ref{lst:minsearch-mch} fixes a non-empty constant subset $\texttt{S} \in \texttt{FIN1(INTEGER)}$ and maintains a set $\texttt{done}$ of already explored elements of \texttt{S} together with a candidate $\texttt{m}$ satisfying $\texttt{m} = \min(\texttt{done})$.
...
\end{lstlisting}
  \end{minipage}
  \caption{Specification \texttt{MinSearch.mch}.}
  \label{lst:minsearch-mch}
\end{figure}

This level already showcases \barrel's handling of partial operators: $\min(\texttt{done})$ and $\min(\texttt{done} \cup \texttt{add})$ are well-defined only if they can be bounded below, hence the POs are generated along with respective WD conditions, that are discharged automatically by \barrel's underlying automation.
Remaining subgoals correspond to standard invariant preservation and operation correctness obligations.

\paragraph*{First refinement: reducingon-determinism.}
...
\begin{lstlisting}[language=b,caption={Refinement \texttt{MinSearch\_r1.ref} (excerpt): refined \texttt{step}.},label={lst:minsearch-r1-step}]
step =
  IF done ≠ S THEN
...
Although the concrete update avoids recomputing \texttt{min}, refinement of the operation and invariant preservation still relate $\texttt{m}$ to $\min(\texttt{done})$, so WD facts about \texttt{min} remain part of the proof landscape.
...
A gluing invariant links the abstract set $\texttt{done}$ with the image $\texttt{tab[1..i]}$ and identifies $\texttt{m}$ with $\texttt{mc}$, and states that $\texttt{mc}$ is a lower bound for all scanned entries.
...
This level is particularly relevant for \barrel because it combines refinement obligations with multiple occurrences of different partial constructs like cardinality \texttt{card} and application of \texttt{tab(i)}.
Correctly discharging the resulting POs requires \barrel to generate and instantiate the corresponding WD conditions for every occurrence of such operators, which makes the number of WD subgoals grow significantly: 72 WD conditions are generated (all automatically discharged) alongside the 21 POs corresponding to standard refinement obligations.
...
Working with \barrel involves standard Lean workflow: B files are imported and generate POs as Lean theorems to be proved, as shown in Listing~\ref{lst:minsearch-lean}.
...
\begin{table}[t]
  ...
  ``Auto'' counts obligations discharged without user interaction (Atelier~B: Force~1; Lean: \barrel default automation). Recall that \barrel's automation is primarily designed to discharge WD conditions.}
  \label{tab:case-study-stats}
  \begin{tabular}{l|cc|cc|cc|cc}
    & \multicolumn{2}{c|}{\#PO (Atelier~B)} & \multicolumn{2}{c|}{\#PO (\barrel)}
    & \multicolumn{2}{c|}{\#WD (Atelier~B)} & \multicolumn{2}{c}{\#WD (\barrel)} \\
    \hline
    File & Total & Auto & Total & Auto & Total & Auto & Total & Auto \\
    \hline
    \texttt{MinSearch}        & 4  & 3 
                              & 4  & 0
                              & 4  & 4
                              & 4  & 4  \\
    \texttt{MinSearch\_r1}    & 19 & 17
                              & 19 & 10
                              & 2 & 2
                              & 42 & 42 \\
    \texttt{MinSearch\_r2}    & 21 & 9
                              & 21 & 0
                              & 9 & 9
                              & 100 & 100 \\
  \end{tabular}
...
Fortunately, \barrel's automation should be able to discharge most of the resulting WD conditions automatically, as showcased in this case study.

\paragraph*{Comparison with Atelier~B's interactive prover.}
...
In practice, however, interactive proof in Atelier~B is primarily carried out through a dedicated, GUI-oriented workflow, where proof progress often depends on manual rule application and instantiation steps, especially for recurrent well-definedness side conditions.
Moreover, the interactive prover documentation explicitly warns that some instantiation commands (e.g.\ \texttt{se}, instantiation of existentially quantified variables) do not enforce typing or well-definedness of user-provided instantiations, potentially leading to inconsistent proofs.
By contrast, BARReL lets users discharge the same obligations as ordinary Lean goals.
In particular, Lean tactics such as \texttt{exists} can only accept type-correct instantiations, hence ill-typed terms are rejected by construction; and under BARReL’s encoding, partial B operators require explicit well-definedness proofs, so ill-defined instantiations cannot even be formed without providing the corresponding evidence.