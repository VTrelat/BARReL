...
This level already showcases \barrel's handling of partial operators: $\min(\texttt{done})$ and $\min(\texttt{done} \cup \texttt{add})$ are well-defined only if they can be bounded below, hence the POs are generated along with respective WD conditions \fixme[uh?], that are discharged automatically by \barrel's underlying automation.
...
\todo[Because we don't really want to only trash talk Atelier B, perhaps include other things that are better handled by Lean? (e.g. reals, and basically everything that comes from mathlib)]