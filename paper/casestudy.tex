...
The development computes the minimum of a non-empty finite set of integers and is progressively refined into an indexed-based traversal of an array,
therefore demonstrating \barrel's support for Atelier~B's refinement pipeline---down to implementation\footnote{Not shown in this development.}---while correctly generating and exposing WD conditions needed for partial B operators at each level.
...
This level already showcases \barrel's handling of partial operators: $\min(S)$ is well-defined only for non-empty finite sets, hence the POs are generated along with respective WD conditions, that are discharged automatically by \barrel's underlying automation.
...
prove_obligations_of MinSearch_ r2
...
\begin{table}[t]
  ...
  \begin{tabular}{l|cc|cc|cc|cc}
    & \multicolumn{2}{c|}{\#PO (Atelier~B)} & \multicolumn{2}{c|}{\#PO (Lean)}
    & \multicolumn{2}{c|}{\#WD (Atelier~B)} & \multicolumn{2}{c}{\#WD (Lean)} \\
    \hline
