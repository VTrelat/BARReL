...
This level already showcases \barrel's handling of partial operators: $\min(\texttt{done})$ and $\min(\texttt{done} \cup \texttt{add})$ are well-defined only for non-empty finite sets, hence the POs are generated along with respective WD conditions, that are discharged automatically by \barrel's underlying automation.
