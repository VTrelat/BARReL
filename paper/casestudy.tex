...
Correctly discharging the resulting POs requires \barrel to generate and instantiate the corresponding WD conditions for every occurrence of such operators, which makes the number of WD subgoals grow significantly: 100 WD conditions are generated (all automatically discharged) alongside the 21 POs corresponding to standard refinement obligations.
...
Moreover, the interactive prover documentation explicitly warns that some instantiation commands, e.g.\@ \texttt{se} for existentially quantified variables, do not enforce typing and well-definedness of user-provided terms, potentially leading to inconsistent proofs
\footnote{
It is actually easy to derive falsehood in Atelier~B, an example being the following assertion: $\exists x, x \in \mathbb{N} \land x \notin \mathbb{Z}$.
Atelier~B's interactive prover accepts the instantiation $x := \min(\varnothing)$ and the proofs completes successfully after a call to the automatic internal prover \texttt{pp}.}.
