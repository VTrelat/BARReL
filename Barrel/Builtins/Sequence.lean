...
namespace B.Builtins
  ...
  structure size.WF {α : Type _} (S : SetRel ℤ α) : Prop where
    isSeq : S ∈ seq (ran S)

  noncomputable abbrev size {α : Type _} (S : SetRel ℤ α) (wf : size.WF S) : ℤ :=
    Classical.choose wf.isSeq

  section Lemmas

    @[grind .]
    theorem size.WF_of_empty {α : Type _} : size.WF (∅ : SetRel ℤ α) where
      isSeq := by
        use 0
        and_intros
        · use 0
    ...
    theorem size.WF_of_mem_seq {α : Type _} {E : Set α} {S : SetRel ℤ α}
        (hS : S ∈ seq E) : size.WF S where
      isSeq := by
        obtain ⟨n, hn, hS'⟩ := hS
        use n, hn
